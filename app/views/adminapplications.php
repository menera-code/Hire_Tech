...
<head>
    ...
    <title><?= $data['title'] ?? 'Admin Applications' ?></title>
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="stats-cards">
            ...
            <div class="stat-card">
                <h3><?= array_reduce($data['applications'], function($carry, $app) {
                    return $carry + ($app['status'] === 'Applied' ? 1 : 0);
                }, 0) ?></h3>
                <p>Pending Review</p>
            </div>
            <div class="stat-card">
                <h3><?= array_reduce($data['applications'], function($carry, $app) {
                    return $carry + ($app['status'] === 'Interview Scheduled' ? 1 : 0);
                }, 0) ?></h3>
                <p>Interviews</p>
            </div>
            <div class="stat-card">
                <h3><?= array_reduce($data['applications'], function($carry, $app) {
                    return $carry + ($app['status'] === 'Hired' ? 1 : 0);
                }, 0) ?></h3>
                <p>Hired</p>
            </div>
        </div>
        ...
        <div class="admin-section">
            ...
            <div class="filter-buttons">
                ...
                <button class="filter-btn">Interview Scheduled</button>
                    ...
                    <tbody>
                        <?php foreach ($data['applications'] as $application): ?>
                            <tr>
                                ...
                                <td>
                                    <div class="applicant-name"><?= htmlspecialchars($application['applicant_name']) ?></div>
                                    <div class="applicant-email"><?= htmlspecialchars($application['applicant_email']) ?></div>
                                </td>
                                <td>
                                    <div class="job-title"><?= htmlspecialchars($application['job_title']) ?></div>
                                    <div class="job-company"><?= htmlspecialchars($application['job_company']) ?></div>
                                </td>
                                <td>
                                    <div class="employer-name"><?= htmlspecialchars($application['employer_name']) ?></div>
                                </td>
                                <td>
                                    <?php
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <a href="/admin/applications?action=delete&id=<?= $application['id'] ?>" 
                ...
                <?php if ($data['total_pages'] > 1): ?>
                    <div class="pagination">
                        <?php if ($data['current_page'] > 1): ?>
                            <a href="/admin/applications?page=<?= $data['current_page'] - 1 ?>">
                        ...
                        <?php for ($i = 1; $i <= $data['total_pages']; $i++): ?>
                            ...
                            <?php else: ?>
                                <a href="/admin/applications?page=<?= $i ?>"><?= $i ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>
            ...
            <?php else: ?>
                <div class="no-data">
                    ...
                    <p>There are no job applications on the platform yet.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
                ...
                if (!hasResults && searchTerm) {
                    ...
                    noResults.innerHTML = `
                        <td colspan="7" style="text-align: center; padding: 40px; color: var(--gray-500);">
                            ...
                            <h3>No applications found</h3>
                            <p>No applications match your search for "<strong>${searchTerm}</strong>"</p>
                        </td>
                    `;
                    applicationsTable.querySelector('tbody').appendChild(noResults);
                }
            }
